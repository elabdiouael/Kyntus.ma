...
function QuantumCore({ isHovered, mouseX, mouseY }: { isHovered: boolean; mouseX: MotionValue<number>; mouseY: MotionValue<number> }) {
  ...
  useFrame((state, delta) => {
    ...
    // L'vitesse katzad x4 f l'Hover (Overdrive Mode)
    ...
    // L'Hbaal dyal l'Morphing 3D
    if (isHovered) {
      meshRef.current.scale.lerp(new THREE.Vector3(1.3, 1.3, 1.3), 0.1);
      wireRef.current.scale.lerp(new THREE.Vector3(1.5, 1.5, 1.5), 0.1);
      material.color.lerp(new THREE.Color("#0044ff"), 0.1); // Deep Blue Metal
      ...
      wireMaterial.color.lerp(new THREE.Color("#ffffff"), 0.1); // Chabaka katwli bida
      ...
      material.color.lerp(new THREE.Color("#ffffff"), 0.1); // Zaja Bida (Default)
      ...
      wireMaterial.color.lerp(new THREE.Color("#2EED2E"), 0.1); // Chabaka khedra
